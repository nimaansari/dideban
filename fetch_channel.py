...
for block in post_blocks:
    ...
    # Extract video — search ALL cdn URLs in block, not just specific classes
    video_url = None
    # Try token URL first (authenticated, doesn't expire as quickly)
    vid_match = re.search(r'(https://cdn\d*\.telesco\.pe/file/[a-z0-9]+\.mp4\?token=[^\s"\'\\<>]+)', block)
    if vid_match:
        video_url = vid_match.group(1)
    else:
        # Fallback: bare .mp4 URL anywhere in block
        vid_match = re.search(r'(https?://cdn\d*\.telesco\.pe/file/[a-z0-9]+\.mp4)', block)
        if vid_match:
            video_url = vid_match.group(1)
